...
CREATE OR REPLACE PROCEDURE add_year_month_partition_column(
    table_name STRING,
...
def run(session, table_name, new_partition_col_name, date_col_name):
    try:
        ...
        update_table_sql = f"UPDATE {table_name} SET {new_partition_col_name} = TO_CHAR({date_col_name}, 'YYYY_MM');"
